...
def register_recruiter_application_routes(
    ...
    def update_applicant_status(job_id, application_id):
        ...
        submission = (
            ...
            .filter_by(id=application_id, job_id=job.id)
            .first()
        )
        ...
        if submission.status != expected_status:
            return error_response(
                ...
                "Reload the applicants page before updating it.",
                409,
            )
        ...
        # Include the previous status in the UPDATE so a concurrent
        # change cannot be silently overwritten.
        owned_job_ids = (
            db.session.query(JobPosting.id)
            .filter(
                ...
                .update(
                    {
                        ...
                        JobApplication.updated_at: datetime.now(
                            timezone.utc
                        ),
                    },
