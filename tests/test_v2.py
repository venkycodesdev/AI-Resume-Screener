...
def clean_database():
    ...
    # Close the setup context before test requests begin.
    ...
    try:
        ...
        # Clean up using a separate application context.
        with application.app.app_context():
            application.db.session.remove()
            application.db.drop_all()
...
"""
    job = (
        "Python Flask developer with REST API, PostgreSQL, "
        "Git and AWS skills."
    )
    ...
    analysis = application.Analysis(
        ...
        job_description=job,
        match_score=application.V2.weighted_score(
            breakdown,
            weights,
        ),
...
def test_saved_reports_are_isolated_by_user(client):
    ...
    # The app's global 404 handler redirects to a safe page.
    # It must never expose or delete another user's report.
...
def test_saved_report_search_and_pdf_download(client):
    ...
    response = client.get(
        "/reports?q=python&min_score=0&type=single"
    )
...
def test_scoring_weights_validation_and_save(client):
    ...
    with application.app.app_context():
        preference = application.V2.models[
            "ScoringPreference"
        ].query.one()
...
def test_resume_rewrite_edit_download_and_reanalyze(client):
    ...
    for file_format, mimetype in formats:
        download = client.get(
            f"/rewrite/{rewrite_id}/download/{file_format}"
        )
